...
const Content = ({
  ...
  return (
    <div className="flex flex-col w-[80%] h-screen px-2">
      <div className="flex justify-between w-full text-xl font-bold text-gray-600 border-b-2 border-indigo-200 pt-6 pb-2 px-6">
        {pageTitle}

        {/* home button */}
        <Link to="/" onClick={() => ToggleAdminMode(dispatch, false)}>
          <button className=" flex items-center justify-center gap-3 text-indigo-700 font-bold py-2 px-4 rounded-lg">
            <FaShopify />
            store
          </button>
        </Link>
